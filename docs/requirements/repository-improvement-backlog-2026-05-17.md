...
| Priority | テーマ | 目的 |
| --- | --- | --- |
| P0 | GitHub 運用の基盤整備 | レビュー漏れと事故防止 |
| P0 | Secret / env 事故予防 | 誤コミット防止 |
| P1 | CI の安全性と見通し改善 | 品質ゲート強化 |
| P1 | 文書とテンプレート整備 | onboarding 改善 |
| P2 | Release / versioning 整備 | 変更履歴の明確化 |
| P2 | テスト拡張 | 回帰検知強化 |
| P3 | アーキテクチャ整理 | 将来拡張のしやすさ向上 |
...
- 上位 1 から 7 までは、比較的低コストで運用事故を減らしやすい。
- 8 以降は、運用の見通しと将来の継続保守性を高めるための整備項目である。
- 実際に Issue を作成する場合は、まず `type/*` `priority/*` ラベル体系を決めてから投入すると運用しやすい。
