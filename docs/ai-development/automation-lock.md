...
親 README と `operations/scheduled-run-lock.md` に合わせ、スケジュールはタスク処理用の 1 本だけを使う方針です。12 本の個別スケジュールは使いません。

GitHub 側にロック JSON ファイルを置く方式は使いません。ロック判定元は ChatGPT 側メモリーです。
