...
親リポジトリの方針に合わせ、現時点では 12 本のスケジュールを有効運用しません。まずは README / `docs/ai-development/` の手順整備、Issue triage、判断回収、handover を小さく進めます。

目指す状態:

- `AGENTS.md`、`docs/ai-development/agent-instructions.md`、`docs/requirements.md`、`docs/ai-development/requirements.md`、`docs/ai-development/work-log.md` の役割が明確である。
- レビュー指摘はそのまま実装へ流さず、Issue / Finding Triage で分類する。
- 実装ジョブは、設計確定済み、未確定事項なし、Open ブロッカーなし、小さく分解済み、検証方法明確な Issue だけを扱う。
- スケジュール登録、PR 作成、プロダクトコード変更、本番操作、破壊的変更、権限変更は、人間承認または追加方針が出るまで行わない。
